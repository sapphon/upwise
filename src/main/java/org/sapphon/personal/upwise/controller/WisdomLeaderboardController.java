...
public class WisdomLeaderboardController {
    ...
    public String getWisdomLeaderboardWithVotes(Model model){
        List<WisdomWithVotes> allWisdoms = getWisdomsWithVotes(wisdomService.getAllWisdoms());
        model.addAttribute("allWisdoms", allWisdoms);
        return "wisdomleaderboard";
    }

    private List<WisdomWithVotes> getWisdomsWithVotes(List<IWisdom> allWisdoms) {
        List<WisdomWithVotes> toReturn = new ArrayList<>();
